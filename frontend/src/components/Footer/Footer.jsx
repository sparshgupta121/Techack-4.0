...
const Footer = () => {
  ...
  return <footer className="pb-16 pt-10">
    <div className="container">
      ...
      <div> 
      <img src={logo} alt="" />
      <p className="text-[16px] leading-7 font-[400] text-textColor">Copyright © {year} developed by Team Hashtag <br />All Rights Reserved</p>


      <div className="flex item-center gap-3 mt-4">
        {socialLinks.map((link,index)=> <Link to={link.path} key={index} className="w-9 h-9 border border-solid border-brown rounded-full flex items-center justify-center group hover:bg-primaryColor hover:border-none ">{link.icon}</Link>)}
      </div>
